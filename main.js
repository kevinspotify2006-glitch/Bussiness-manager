...
function paintVehicle(ctx, raw, x, y, heading, length, width, detail) {
  const a = face(raw);
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate(heading);
...
var SKILLS = [
  ...
  { id: "admin", name: "Administration", detail: "Paperwork, ordering, money, and keeping the place legal." },
...
var ROLE_SKILLS = {
  cashier: { service: 0.45, sales: 0.3, admin: 0.25 },
  sales: { sales: 0.55, service: 0.32, admin: 0.13 },
  server: { service: 0.58, sales: 0.24, logistics: 0.18 },
  cook: { technical: 0.52, logistics: 0.26, service: 0.22 },
  technician: { technical: 0.72, service: 0.16, admin: 0.12 },
  cleaner: { service: 0.48, technical: 0.32, logistics: 0.2 },
  warehouse: { logistics: 0.72, technical: 0.16, admin: 0.12 },
  driver: { logistics: 0.78, service: 0.12, technical: 0.1 },
  marketer: { sales: 0.56, admin: 0.24, service: 0.2 },
  accountant: { admin: 0.74, technical: 0.14, leadership: 0.12 },
  manager: { leadership: 0.54, admin: 0.24, service: 0.22 },
  hr: { leadership: 0.42, admin: 0.34, service: 0.24 },
  it: { technical: 0.66, admin: 0.22, logistics: 0.12 },
  legal: { admin: 0.68, leadership: 0.2, service: 0.12 }
};
function emptySkills() {
  return { technical: 0, sales: 0, service: 0, logistics: 0, admin: 0, leadership: 0 };
}
...
function performanceOf(employee) {
  ...
  const value = base2 * fitFactor(fit) * moraleFactor(employee.morale) * workloadFactor(employee.workload) * experienceFactor(employee.experience) * traits;
...
var COURSES = [
  ...
  { id: "leadership", name: "Leadership", skill: "leadership", days: 8, costPerDay: 290, detail: "Running a shift, and later a shop. The step before a manager." }
];
...
function startResearch(state, id) {
  ...
  state.research.active = { id, endsOnDay: state.day + node2.days };
  return { ok: true, message: `${node2.name} started. ${node2.days} days.` };
}
...
function researchFactor(state, effect) {
  ...
  for (const id of state.research?.done ?? []) {
    const node2 = RESEARCH_BY_ID.get(id);
    if (node2 && node2.effect === effect) factor *= 1 + node2.amount;
  }
  return factor;
}
...
function employeesDaily(state) {
  for (const employee of [...state.employees]) {
    ...
    employee.stress = clamp(employee.stress + pressure * 0.5 - 2 + gameRng.range(-2, 2), 0, 100);
    ...
    const risk = clamp((35 - employee.morale) / 260 + (30 - employee.loyalty) / 900, 0, 0.22) * retentionFactor(state);
...
function functionState(state, id) {
  ...
  if (outsourced) {
    return { def, people: people2, needed, cover: def.outsourceEffect, outsourced: true, cost: outsourceCost(state, def) };
  }
  if (!hq || people2.length === 0) {
    return { def, people: people2, needed, cover: 0, outsourced: false, cost: 0 };
  }
  const quality = sum(people2, (person) => performanceOf(person) / 70) / people2.length;
  const cover2 = clamp(people2.length / needed * clamp(quality, 0.4, 1.15), 0, 1);
  return { def, people: people2, needed, cover: cover2, outsourced: false, cost: sum(people2, (p2) => p2.salary) };
}
...
function coordinationFactor(state) {
  return clamp(1 - cover(state, "finance") * 0.45 - cover(state, "it") * 0.25, 0.3, 1);
}
...
function headOfficeDaily(state) {
  const bill = outsourcingBill(state);
  if (bill <= 0) return;
  post(state, state.playerCompanyId, "outsourcing", "Outsourced central services", -bill / 30, null);
}
...
function borrowingHeadroom(state) {
  ...
  return Math.max(0, (secured + 25e3 * rating) * (0.6 + rating) * standing - debtTotal(state));
}
...
var SIZES = [
  ...
  { id: "medium", name: "Medium", from: 6, managers: 1, adminPerHead: 1.6, detail: "Somebody has to run the rota now." },
  { id: "large", name: "Large", from: 14, managers: 2, adminPerHead: 3.4, detail: "Shifts, handovers, and people who never meet." },
  { id: "enterprise", name: "Enterprise", from: 26, managers: 3, adminPerHead: 5.6, detail: "A business that runs whether or not you are in it." }
];
...
function managementCover(state, business) {
  ...
  if (size.managers === 0) return 1;
  const managers = employeesOf(state, business.id).filter((person) => person.role === "manager").length;
  return clamp(managers / size.managers, 0, 1.25);
}
...
function adminCostPerDay(state, business) {
  const size = sizeOf(state, business);
  const headcount = employeesOf(state, business.id).length;
  return size.adminPerHead * headcount * coordinationFactor(state);
}
function staffingPlan(state, business) {
  ...
  const size = sizeOf(state, business);
  const relief = automationRelief(state);
  const rows = [];
  for (const [index, roleId] of type.roles.entries()) {
    ...
    let recommended = 1;
    if (roleId === "manager") {
      required = Math.max(1, size.managers);
...
function findings(state, business) {
  ...
  if (plan.managementCover < 1 && plan.size.managers > 0) {
    out.push({
      level: "warn",
      title: `A ${plan.size.name.toLowerCase()} business needs ${plan.size.managers} manager${plan.size.managers === 1 ? "" : "s"}`,
      detail: `${plan.size.detail} Without them everything still happens, about ${Math.round((1 - managementFactor(state, business)) * 100)}% slower.`
    });
  }
...
function opportunities(state, business) {
  ...
  const ready = staff.find((person) => person.role !== "manager" && person.skill > 68 && person.promotions < 2);
  if (ready) {
    out.push({
      level: "good",
      title: `${ready.name} has earned a step up`,
      detail: "Recognise them before somebody else does. Promotion buys loyalty that money alone does not."
    });
  }
  const size = sizeOf(state, business);
  if (size.id !== "enterprise" && staff.length >= SIZES[SIZES.indexOf(size) + 1]?.from - 2) {
    out.push({
      level: "good",
      title: `Two more people and this is a ${SIZES[SIZES.indexOf(size) + 1].name.toLowerCase()} business`,
      detail: SIZES[SIZES.indexOf(size) + 1].detail
    });
  }
  return out;
}
...
function unitPrice(state, supplierId, productId, districtEventCost = 1) {
  ...
  return Number(
    (productDef.wholesalePrice * def.priceMultiplier * (1 - discount) * state.economy.inflation * districtEventCost * buying).toFixed(2)
  );
}
...
function expectedDailyAwareness(state, channelId, businessId) {
  ...
  return channel.reach * channel.conversion * match * headroom * 100 * 0.42 * marketingFactor(state);
}
...
function people(input) {
  ...
  return input.filter(isRecord).map((entry) => {
    ...
    if (!isRecord(entry.skills)) {
      ...
      const skills = emptySkills();
      for (const id of SKILL_IDS) skills[id] = num(saved[id], num(entry.skill, 50));
...
function sameParams(a, b) {
  ...
  for (const key2 of keys) if (a[key2] !== b[key2]) return false;
  return true;
}
var App = class {
  engine;
  ...
  /**
   * Goes somewhere. This is navigation, so the new screen starts at the top —
   * which is right when the player asked for a different screen, and wrong for
   * everything else, so everything else goes through `refresh`.
   */
  go(route2, params = {}) {
    const entry = this.entries.find((item) => item.route === route2) ?? this.entries[0];
    if (!entry) return;
    const movedScreen = entry.route !== this.route || !sameParams(params, this.params);
...
function headOfficeView(ctx) {
    ...
    el.appendChild(
      section(
        ...
        h(
          ...
          stat("Central staff", String(staff.length)),
          stat("Supporting", `${locations} location${locations === 1 ? "" : "s"}`),
          stat("Bought in", outsourcingBill(state) > 0 ? `${money(outsourcingBill(state))}/mo` : "nothing")
        )
      )
    );
        ...
        h(
          ...
          h(
            ...
            button(
              ...
              row2.outsourced ? "btn small" : "btn small"
            )
          )
        )
      );
  ...
  const signature = () => {
    const state = ctx.state;
    return `${state.day}|${state.headOffice?.id ?? ""}|${centralStaff(state).length}|${outsourcingBill(state)}|${state.applicants.length}`;
  };
  render();
  let shown = signature();
  return {
...
function financeView(ctx) {
  ...
  const peopleDays = 30;
  const since = state.day - peopleDays;
  const peopleSpend = /* @__PURE__ */ new Map();
  for (const entry of state.ledger) {
    if (entry.day < since) continue;
    if (!peopleLines.some((row2) => row2.label === entry.category)) continue;
    peopleSpend.set(entry.category, (peopleSpend.get(entry.category) ?? 0) + -entry.amount);
  }
  const peopleTotal = sum([...peopleSpend.values()], (value) => value);
  const revenue30 = sum(state.dayHistory.slice(-peopleDays), (day) => day.revenue);
  el.appendChild(
    section(
      "What the people cost \u2014 last 30 days",
      peopleTotal <= 0 ? empty("Nothing yet. The payroll appears here once somebody is working for you.") : table(
        ["", "Amount", "Of turnover"],
        [
          ...peopleLines.filter((row2) => (peopleSpend.get(row2.label) ?? 0) > 0).map((row2) => [
            row2.text,
            money(peopleSpend.get(row2.label) ?? 0),
            revenue30 > 0 ? pct((peopleSpend.get(row2.label) ?? 0) / revenue30 * 100) : "\u2014"
          ]),
          [
            h("strong", { text: "Everything the people cost" }),
            h("strong", { text: money(peopleTotal) }),
            h("strong", { text: revenue30 > 0 ? pct(peopleTotal / revenue30 * 100) : "\u2014" })
          ]
        ]
      )
    )
...
function boot(state) {
  ...
  globalThis.__BM__ = { engine, state, app, map: () => CityMap.current };
