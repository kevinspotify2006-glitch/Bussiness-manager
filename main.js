...
function buildGraph() {
  ...
  for (const seg of segments) {
    ...
    for (const t of stops) {
      ...
      if (previous >= 0) link2(previous, id, seg.cls, seg.width);
      previous = id;
    }
  }
  return { nodes, edges };
...
var CHRONICLE_LIMIT = 400;
function record(state, kind, text, options = {}) {
...
function recordRows(state) {
  ...
  const when = (day) => day > 0 ? calendar(day).label : "not yet";
  return [
...
function recruitmentFee(state) {
  return Math.round(RECRUITMENT_FEE * (1 - cover(state, "hr") * 0.5) * employerPull(state).fee);
}
...
// src/sim/workingCapital.ts
function tradeCreditOf(business) {
  const type = businessTypeOrThrow(business.typeId);
  const share = type.tradeCredit ?? 0;
  return { share: clamp(share, 0, 1), days: type.tradeCreditDays ?? 30 };
}
function contractTerms(value) {
  if (value >= 4e4) return 60;
  if (value >= 12e3) return 45;
  return 30;
}
var handingOver = false;
function setHandoverMode(on2) {
  handingOver = on2;
}
function supplierTerms(state, supplierId) {
  if (handingOver) return 0;
  const def = supplier(supplierId);
  if (!def) return 0;
  const rating = financialStanding(state);
  if (rating < 35) return 0;
  const base2 = def.reliability > 0.95 ? 14 : 21;
  if (rating >= 75) return base2 + 14;
  if (rating >= 55) return base2;
  return Math.max(7, base2 - 7);
}
function bookTakings(state, business, amount, label) {
  if (amount <= 0) return;
...
function raiseInvoice(state, options) {
...
function raiseBill(state, options) {
  if (options.amount <= 0) return;
  state.bills.push({
    id: makeId("bill"),
...
function workingCapitalDaily(state) {
  const company = playerCompany(state);
  let collected = 0;
  let paid = 0;
  let missed = 0;
...
function workingCapital(state) {
  ...
  const settled = state.invoices.length > 0 ? sum(state.invoices, (i) => state.day - i.raisedOnDay) / state.invoices.length : 0;
  return {
    receivables,
    overdue,
    payables,
...
function dailyOutgoingsEstimate(state) {
  const recent = state.dayHistory.slice(-14);
  if (recent.length === 0) return 0;
  return Math.max(0, sum(recent, (d) => d.costs) / recent.length);
}
function checkLiquidity(state) {
  ...
  pushAlert(
    ...
    owed > company.cash ? `${money(owed)} is owed to you against ${money(company.cash)} in the bank. The money exists; it is just not here yet. Chase it, shorten your terms, or borrow against it.` : `${money(company.cash)} in the bank against ${money(dailyOutgoingsEstimate(state))} a day going out.`,
    null,
    "finance"
  );
}
function reserveTax(state, monthProfit, rate) {
  state.taxReserve = Math.max(0, Math.round(monthProfit * rate));
}
function releaseTaxReserve(state, paid) {
  state.taxReserve = Math.max(0, state.taxReserve - Math.round(paid));
}
var FACTORING_FEE = 0.045;
function factorInvoices(state, upToAmount) {
  ...
  for (const invoice of due) {
    if (face2 >= upToAmount) break;
    face2 += invoice.amount;
    taken.push(invoice.id);
  }
  if (face2 <= 0) return { ok: false, message: "Nothing on the sales ledger to advance against.", raised: 0 };
  const fee = Math.round(face2 * FACTORING_FEE);
  const raised = face2 - fee;
  const company = playerCompany(state);
  ...
  pushNews(
    ...
    `${money(face2)} of invoices advanced for ${money(raised)}. The ${money(fee)} difference is what it costs to have the money now instead of in ${due[0] ? Math.max(0, due[0].dueOnDay - state.day) : 30} days.`,
    { importance: "high" }
  );
  return {
    ok: true,
...
function ageing(state) {
  return state.invoices.map((invoice) => ({ invoice, daysLate: state.day - invoice.dueOnDay })).sort((a, b) => b.daysLate - a.daysLate);
}
function duesoon(state) {
...
function netWorth(state) {
  return playerCompany(state).cash + receivablesValue(state) + inventoryValue(state) + propertyValue(state) + goodwillValue(state) - payablesValue(state) - debtTotal(state);
}
function receivablesValue(state) {
  return sum(state.invoices, (invoice) => invoice.amount);
}
function payablesValue(state) {
...
function monthlyProfit(state, days) {
  ...
  let revenue = 0;
  let costs = 0;
  for (const record2 of state.dayHistory) {
    if (record2.day <= from) continue;
    revenue += record2.revenue;
    costs += record2.costs;
  }
...
function creditFactors(state) {
  ...
  const rows = [
    {
      ...
      detail: arrears.length > 0 ? `${arrears.length} bill${arrears.length === 1 ? "" : "s"} late, ${lateDays} days in total.` : billsEver === 0 ? "Nothing bought on account yet, so there is no record to go on." : "Nothing outstanding past its date."
    },
    {
      label: "Loan payments",
      of: 20,
      score: clamp(20 - missed * 6, 0, 20) * proven,
      detail: missed > 0 ? `${missed} missed payment${missed === 1 ? "" : "s"} on the record.` : state.loans.length === 0 ? "Never borrowed, so nothing to judge." : "Every payment made on time."
    },
    {
      label: "Borrowing against assets",
      of: 20,
      ...
      detail: worth <= 0 ? "The company owes more than it owns." : `${Math.round(leverage * 100)}% of what the company owns is borrowed.`
    },
    {
      label: "Cash reserve",
      of: 15,
      score: clamp(months * 7, 0, 15),
      detail: company.cash < 0 ? "The account is overdrawn." : Number.isFinite(months) ? `${months.toFixed(1)} months of outgoings in the bank.` : "No outgoings to measure against yet."
    },
    {
      label: "Trading profitably",
      of: 15,
  ...
  const total2 = sum(rows, (r) => r.score);
  if (total2 > ceiling) {
    const scale = ceiling / total2;
    for (const row2 of rows) row2.score *= scale;
    const capped = rows.find((r) => r.label === "Cash reserve");
    if (capped) {
      capped.detail = company.cash < 0 ? "The account is overdrawn, which caps the whole rating." : "The company owes more than it owns, which caps the whole rating.";
    }
  }
  return rows;
}
...
var MAX_BONDS = 3;
function bondsOf(employee) {
  return employee.bonds ?? [];
}
function bondWith(employee, otherId) {
  return bondsOf(employee).find((b) => b.withId === otherId);
}
function shopDaily(state, business) {
  ...
  const leadership = leaders.length > 0 ? sum(leaders, (person) => person.skills.leadership) / leaders.length : 25;
  ...
  const traitsOf = (person) => person.traits.map(trait);
  const friction = (person) => sum(traitsOf(person), (t) => -t.teamMorale);
  const generous = (person) => sum(traitsOf(person), (t) => t.teamMorale);
...
function deepen(state, a, b, kind, by) {
  link(state, a, b, kind, by);
  link(state, b, a, kind === "mentor" ? "mentor" : kind, by);
}
function link(state, from, to, kind, by) {
...
function announceFriction(state, a, b) {
...
function announceRepair(state, a, b) {
  if (performanceOf(a) < 60 && performanceOf(b) < 60) return;
  pushNews(
    state,
    "staff",
...
function relationshipsDaily(state) {
  for (const business of playerBusinesses(state)) {
    if (business.status !== "open") continue;
...
function bondMorale(state, employee) {
  ...
  for (const bond of bonds) {
    ...
    total2 += bond.strength > 0 ? bond.strength * 0.05 : bond.strength * 0.085;
  }
  return clamp(total2, -14, 8);
}
function mentorBoost(state, employee) {
  let best = 0;
...
function teamFriction(state, business) {
...
function relationshipsOf(state, employee) {
  return bondsOf(employee).map((bond) => ({ other: employeeById(state, bond.withId), bond })).filter((row2) => row2.other !== void 0).sort((a, b) => Math.abs(b.bond.strength) - Math.abs(a.bond.strength));
}
function describeBond(bond, other) {
...
function duplicatesIn(state, divisionId) {
  ...
  for (const business of divisionBusinesses(state, divisionId)) {
    ...
    const summary2 = staffingSummary(state, business);
    const keepTotal = Math.max(summary2.required, Math.round(summary2.recommended * (1 - SHARED_SERVICES)));
...
function researchDaily(state) {
  ...
  record(state, "research", `Completed ${node2.name}`, { amount: -node2.cost });
...
function generateApplicant(state, forcedRole) {
  ...
  const skill = Math.round(clamp(gameRng.around(52 + employerPull(state).skill, 26), 8, 99));
  let startGrade = 0;
...
function employeesDaily(state) {
  for (const employee of [...state.employees]) {
    ...
    const settlesAt = clamp(
      68 + moraleFromOwner(state) + mergerMoraleDrag(state, employee) + payEffect * 1.6 + moraleDrift * 10 + fromColleagues * 8 + // And the particular people they work beside, which is not the same
      // thing as the average of everybody's temperament.
      bondMorale(state, employee) - Math.max(0, employee.workload - 60) * 0.5 - employee.stress * 0.22,
      0,
      100
    );
...
function standingTargets(state) {
  ...
  const stars2 = reviewed.length > 0 ? sum(reviewed, (b) => b.reviewScore) / reviewed.length : 3;
  const run = open.length > 0 ? sum(open, (b) => b.serviceQuality) / open.length : 50;
  const quality = clamp((stars2 - 1) / 4 * 70 + run * 0.3, 0, 100);
  const cover2 = open.length > 0 ? sum(open, (b) => {
  ...
  const service = clamp(cover2 * 82 - Math.max(0, strain - 70) * 1.2, 0, 100);
  const paid = staff.length > 0 ? sum(staff, (e) => e.salary / Math.max(1, marketRate(state, e))) / staff.length : 1;
  const morale = staff.length > 0 ? sum(staff, (e) => e.morale) / staff.length : 50;
  const employer = staff.length === 0 ? 50 : clamp(42 + (paid - 1) * 90 + (morale - 55) * 0.55, 0, 100);
...
function standingDaily(state) {
  ...
  state.standing = now;
  announce2(state, "employer", before.employer, now.employer, 70, "People are starting to apply to you unprompted", "A good name as an employer means better applicants at a lower fee.");
  announce2(state, "employer", before.employer, now.employer, 35, "Word has got round that this is a bad place to work", "Worse applicants, and recruiters cost more.", true);
  announce2(state, "quality", before.quality, now.quality, 75, "You have a name for doing good work", "Reviews will run better than the day itself deserves.");
  announce2(state, "financial", before.financial, now.financial, 35, "Your suppliers have started talking to each other", "Terms shorten, and some will want cash up front.", true);
}
function announce2(state, id, before, after, line, title, body, falling = false) {
...
function financialStanding(state) {
...
function pullOf(state, business) {
  ...
  const repeat = business.companyId === state.playerCompanyId ? retentionFactorOf(state) : 1;
  return {
    base: location * scale * reputation * awareness * repeat,
...
function openBusiness(state, businessId) {
  ...
  record(state, "opened", `Opened ${business.name}`, { businessId: business.id });
...
function tradeHour(state, business, allocation) {
  ...
  if (revenue > 0) {
    bookTakings(state, business, revenue, `Takings \u2014 ${business.name}`);
...
function quoteOrder(state, supplierId, businessId, requested) {
  ...
  const onTerms = supplierTerms(state, supplierId) > 0 && exposureTo(state, supplierId) + total2 <= creditCeiling(state, supplierId);
  if (!onTerms && total2 > playerCompany(state).cash) {
    problems.push("Not enough cash for this order.");
  } else if (onTerms && total2 > availableToSpend(state) + creditCeiling(state, supplierId)) {
    problems.push(`${def?.name ?? "This supplier"} will not extend you that much credit on top of what you already owe.`);
  }
  return {
    lines,
    units,
...
function placeOrder(state, supplierId, businessId, requested) {
  ...
  state.supplierSpend[supplierId] = (state.supplierSpend[supplierId] ?? 0) + quote.total;
  const terms = business.companyId === state.playerCompanyId && exposureTo(state, supplierId) + quote.goodsCost <= creditCeiling(state, supplierId) ? supplierTerms(state, supplierId) : 0;
  if (terms > 0) {
    postNonCash(state, business.companyId, "stock", `Stock on account \u2014 ${def.name}`, -quote.goodsCost, businessId);
    raiseBill(state, {
      supplierId,
      to: def.name,
      amount: quote.goodsCost,
      days: terms,
      businessId
    });
  } else {
...
function expectedDailyAwareness(state, channelId, businessId) {
  ...
  return channel.reach * channel.conversion * match * headroom * 100 * 0.42 * marketingFactor(state) * marketingSkillFactor(state) * brandFactor(state);
}
...
function invoiceClient(state, client, amount, what) {
  if (amount <= 0) return;
...
function closeMonth(state, days = 30) {
  ...
  const trading = state.dayHistory.filter((record2) => record2.day >= from && record2.day <= to);
  ...
  const account = {
    ...
    customers: sum(trading, (record2) => record2.customers),
    cash: company.cash,
...
function people(input, version) {
  ...
  return input.filter(isRecord).map((entry) => {
    const employee = {
      ...
      bonds: Array.isArray(entry.bonds) ? entry.bonds.filter(
...
function migrate(input) {
  ...
  const state = {
    ...
    standing: (() => {
      const saved = isRecord(input.standing) ? input.standing : {};
      const base2 = emptyStanding();
      for (const facet of STANDING_FACETS) base2[facet.id] = Math.max(0, Math.min(100, num(saved[facet.id], base2[facet.id])));
      return base2;
    })(),
    ...
    chronicle: Array.isArray(input.chronicle) ? input.chronicle.slice(-400) : [],
    records: (() => {
      const saved = isRecord(input.records) ? input.records : {};
      const base2 = emptyRecords();
      for (const key2 of Object.keys(base2)) {
        const entry = isRecord(saved[key2]) ? saved[key2] : {};
        base2[key2] = { value: num(entry.value, 0), day: num(entry.day, 0) };
      }
      return base2;
    })(),
    taxReserve: Math.max(0, num(input.taxReserve, 0)),
    liquidityBand: input.liquidityBand === "warning" || input.liquidityBand === "critical" ? input.liquidityBand : "clear",
...
function severityTile(level, cls = "advice-dot") {
  const glyph = {
    critical: "critical",
    bad: "critical",
    warning: "warning",
...
var App = class {
  ...
  build() {
    ...
    const mark2 = h("div", { class: "brand-mark" });
    mark2.appendChild(icon("brand", 17));
    const brand = h(
      "div",
      { class: "brand" },
      mark2,
  ...
  renderTopbar() {
    ...
    hud.appendChild(metric2("Cash", "hud-cash"));
    hud.appendChild(metric2("Profit, 30 days", "hud-month"));
    hud.appendChild(metric2("Date", "hud-date"));
  ...
  updateChrome() {
    ...
    if (initials) {
      initials.textContent = (founder?.name ?? "You").split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]?.toUpperCase() ?? "").join("") || "Y";
    }
...
function build2(options) {
  ...
  handover.cash = Math.max(1e3, Math.round(def.cash));
...
function showWelcome(root2, onStart) {
    ...
    card.appendChild(
      h(
        ...
        h(
          ...
          h("p", { class: "welcome-sub", text: "Build \xB7 Manage \xB7 Expand \xB7 Dominate" }),
          h("p", {
            class: "tiny muted",
  ...
  function renderScenario(card) {
    ...
    const grid2 = h("div", { class: "scenario-grid" });
    for (const def of SCENARIOS) {
      ...
      grid2.appendChild(
        h(
          ...
          def.headOffice || def.debtShare > 0 || def.extraRivals > 0 ? fact(
            "warning",
            [
  ...
  function renderCustom() {
    ...
    const grid2 = h("div", { class: "grid cols-3" });
    grid2.appendChild(field("Starting cash", "What is in the bank on day one.", "cash", 5e3, 5e6, 5e3));
    grid2.appendChild(field("Locations", "Opened and trading before you arrive.", "locations", 0, 12, 1));
    grid2.appendChild(field("Staff per location", "Hired into each of them.", "staffPerLocation", 0, 10, 1));
    grid2.appendChild(field("Reputation", "What customers already think, 0\u2013100.", "reputation", 0, 100, 5));
    grid2.appendChild(field("Awareness", "How many people have heard of you, 0\u2013100.", "awareness", 0, 100, 5));
    grid2.appendChild(field("Credit rating", "What the bank thinks, 0\u2013100.", "creditRating", 0, 100, 5));
    grid2.appendChild(field("Days of history", "Trading run before you take over.", "warmUpDays", 0, 120, 5));
    grid2.appendChild(field("Extra rivals", "How crowded the city already is.", "extraRivals", 0, 10, 1));
    grid2.appendChild(field("Level reached", "What the company counts as having earned.", "levelReached", 1, 8, 1));
...
function fact(name, value, detail) {
  const row2 = h("div", { class: "scenario-fact" });
  row2.appendChild(icon(name, 13));
  row2.appendChild(h("span", {}, h("strong", { text: value }), h("span", { class: "muted", text: ` \u2014 ${detail}` })));
  return row2;
}
...
function node(tag2, attrs) {
  const el = document.createElementNS(NS, tag2);
  for (const [key2, value] of Object.entries(attrs)) el.setAttribute(key2, value);
  return el;
}
function grid(el, width, height, pad) {
  for (let i = 0; i <= 4; i += 1) {
...
function lineChart(values, options = {}) {
  ...
  const stroke = last >= 0 ? C2.good : C2.bad;
...
function multiChart(series, options = {}) {
  const width = 600;
...
function dashboardView(ctx) {
  ...
  el.appendChild(
    h(
      ...
      kpi({
        ...
        valueTone: company.cash < 0 ? "bad" : void 0,
        sub: cashAMonthAgo === null ? `${open.length} trading \xB7 ${staffCount(state)} on the payroll` : change(company.cash, cashAMonthAgo),
        subTone: cashAMonthAgo === null ? void 0 : company.cash >= cashAMonthAgo ? "good" : "bad"
      }),
      kpi({
        ...
        subTone: prevRevenue === 0 ? void 0 : monthRevenue >= prevRevenue ? "good" : "bad"
      }),
      kpi({
        label: "Profit, 30 days",
        value: moneySigned(monthProfit),
        icon: "profit",
        ...
        subTone: prevProfit === 0 ? void 0 : monthProfit >= prevProfit ? "good" : "bad"
      }),
      kpi({
        label: "Employees",
        ...
        subTone: hiredRecently > 0 ? "good" : void 0
      })
    )
  );
  el.appendChild(
    h("p", {
      class: "tiny muted",
      style: "margin:-6px 0 0",
      text: `${company.name} \xB7 ${cal.label} \xB7 ${clockLabel(state.hour)} \xB7 economy: ${economy.label}`
    })
  );
  ...
  const adviceRow = (item, star) => h(
    ...
    star ? h("span", { class: "advice-star", text: "\u2605" }) : severityTile(item.severity),
    h(
      "div",
  ...
  for (const row2 of standingRows(state)) {
    standingPanel.appendChild(
      h(
        "div",
        { class: "meter" },
        h("span", { class: `meter-dot ${row2.tone}` }),
        h("span", { class: "meter-name", text: row2.name }),
        h("div", { class: "meter-bar" }, bar(row2.value / 100, row2.tone)),
        h("span", { class: "meter-value", text: String(Math.round(row2.value)) })
      )
    );
    standingPanel.appendChild(h("div", { class: "tiny muted", style: "margin:-4px 0 6px 20px", text: row2.drives }));
  }
  const left = h("div", { class: "list" });
  const right = h("div", { class: "list" });
  const window2 = state.dayHistory.slice(-45);
  const trading = [
    { label: "Revenue", values: window2.map((d) => d.revenue), color: SERIES_COLORS.info, fill: true },
    { label: "Costs", values: window2.map((d) => d.costs), color: SERIES_COLORS.bad },
    { label: "Profit", values: window2.map((d) => d.profit), color: SERIES_COLORS.good }
  ];
  ...
  chartPanel.appendChild(
    h("p", {
      class: "tiny muted",
      text: state.dayHistory.length === 0 ? "Nothing has been settled yet. Press play and let a day run." : `Last ${Math.min(45, state.dayHistory.length)} days. Best day ${money(Math.max(...state.dayHistory.map((d) => d.profit)))}, worst ${money(Math.min(...state.dayHistory.map((d) => d.profit)))}.`
    })
  );
...
function progressView(ctx) {
  ...
  const render = () => {
    ...
    records.appendChild(
      table(
        ["What", "How much", "When"],
        recordRows(state).map((row2) => [row2.label, row2.value, h("span", { class: "tiny muted", text: row2.when })])
      )
    );
...
function employeesView(ctx) {
  ...
  el.appendChild(
    h(
      ...
      kpi({
        label: "Wage bill",
        value: `${money(sum(staff, (e) => e.salary))}/mo`,
        icon: "cash",
        tone: "warn",
      ...
      table(
        ...
        staff.map((employee) => [
          ...
          h(
            "div",
            { class: "cell-person" },
            avatar(employee.name, employee.morale < 40 ? "warn" : employee.morale > 70 ? "good" : void 0),
            h(
              "div",
              { style: "min-width:0" },
...
function openEmployeeDialog(ctx, employee) {
  ...
  if (bonds.length > 0) {
    const people2 = h("div", { class: "panel" }, h("h3", { class: "panel-title", text: "Who they work with" }));
    for (const { other, bond } of bonds) {
      ...
      people2.appendChild(
        h(
          "div",
          { class: "staff-row" },
          avatar(other.name, described.tone === "bad" ? "warn" : void 0, true),
          h(
            "div",
            { style: "flex:1;min-width:0" },
            h("div", { class: "staff-name", text: described.text }),
            h("div", { class: "tiny muted", text: `${titleOf(other)} \xB7 ${roleTitle(other)}` })
          ),
          tag(
    ...
    people2.appendChild(
      h("p", {
        class: "tiny muted",
        text: bonds.some((row2) => row2.bond.strength < 0) ? "Friction costs morale on both sides and the customers notice it. Somebody senior enough to sort it out is the usual answer." : "People who work well together are happier, stay longer, and serve customers better."
      })
    );
    body.appendChild(people2);
  }
  const skillPanel = h(
    "div",
    { class: "panel" },
...
function orderPanel(ctx, business) {
  ...
  const refreshSummary = () => {
    ...
    for (const problem of quote.problems) {
      summary2.appendChild(h("p", { class: "tiny bad", text: problem }));
    }
    summary2.appendChild(
      h(
        "div",
    ...
    const orderButton = summary2.querySelector(".btn.primary");
...
function financeView(ctx) {
  ...
  if (owed > 0 || owing > 0) {
    const ledger2 = section("The sales and purchase ledgers");
    ledger2.appendChild(
      h("p", {
        class: "sub",
        style: "margin:-4px 0 12px",
        text: owed > company.cash ? `More is owed to you than you are holding. The money exists \u2014 it is sitting with your customers. You can chase it, sell the ledger to a lender at a discount, or borrow against it.` : "What has been earned but not yet collected, and what has been bought but not yet paid for."
      })
    );
    const rows2 = ageing(state).slice(0, 8);
    if (rows2.length > 0) {
      ledger2.appendChild(h("div", { class: "group-heading", text: "Owed to you, worst first" }));
      ledger2.appendChild(
        table(
          ["Customer", "Amount", "Raised", "Due", "Status"],
          rows2.map((row2) => [
            row2.invoice.from,
            money(row2.invoice.amount),
            `day ${row2.invoice.raisedOnDay}`,
            `day ${row2.invoice.dueOnDay}`,
            row2.daysLate > 0 ? tag(`${row2.daysLate}d late`, row2.daysLate > 14 ? "bad" : "warn") : tag(`due in ${-row2.daysLate}d`, "good")
          ])
        )
      );
      ledger2.appendChild(
    ...
    if (bills.length > 0) {
      ledger2.appendChild(h("div", { class: "group-heading", text: "You owe, soonest first" }));
      ledger2.appendChild(
        table(
          ...
          bills.map((bill) => [
            ...
            money(bill.amount),
            `day ${bill.dueOnDay}`,
            bill.lateDays > 0 ? tag(`${bill.lateDays}d late`, "bad") : tag(`in ${bill.dueOnDay - state.day}d`, "muted")
          ])
        )
      );
    }
    el.appendChild(ledger2);
  }
  ...
  rating.appendChild(
    h(
      ...
      h("div", {}, ...creditFactors(state).map(
        (factor2) => h(
          "div",
          { class: "meter" },
          h("span", { class: `meter-dot ${factor2.score / factor2.of > 0.7 ? "good" : factor2.score / factor2.of < 0.35 ? "bad" : "warn"}` }),
          h("span", { class: "meter-name", text: factor2.label }),
          h("div", { class: "meter-bar" }, bar(factor2.score / factor2.of, factor2.score / factor2.of > 0.7 ? "good" : factor2.score / factor2.of < 0.35 ? "bad" : "warn")),
          h("span", { class: "meter-value", text: `${Math.round(factor2.score)}/${factor2.of}` })
        )
      ))
    )
  );
  for (const factor2 of creditFactors(state)) {
    rating.appendChild(h("div", { class: "tiny muted", text: `${factor2.label}: ${factor2.detail}` }));
  }
...
function openFactoring(ctx) {
  const state = ctx.state;
  const wc = workingCapital(state);
  const face2 = wc.receivables + wc.overdue;
  const fee = Math.round(face2 * FACTORING_FEE);
  void confirmDialog(
    "Sell the sales ledger",
    `A lender will advance ${money(face2 - fee)} against ${money(face2)} of invoices, today. The ${money(fee)} difference is theirs, and lenders notice a company that does this \u2014 your rating takes a small knock.

The money is already yours; this is only about having it sooner.`,
    `Take ${money(face2 - fee)} now`
  ).then((yes) => {
    if (!yes) return;
    const result = factorInvoices(state, face2);
    toast(result.message, result.ok ? "good" : "bad");
    ctx.refresh();
  });
}
...
function reportsView(ctx) {
  ...
  headline.appendChild(
    h(
      "div",
      { class: "grid cols-4" },
      metric("Revenue", money(yesterday.revenue), previous ? delta(yesterday.revenue, previous.revenue) : "", void 0, "revenue", "info"),
      metric("Costs", money(yesterday.costs), previous ? delta(yesterday.costs, previous.costs, true) : "", void 0, "cash", "warn"),
      ...
      metric("Customers", count(yesterday.customers), previous ? delta(yesterday.customers, previous.customers) : "", void 0, "people", "accent")
    )
  );
  if (previous) {
    ...
    el.appendChild(
      section(
        ...
        h(
          "div",
          { class: "grid cols-4" },
          metric("Revenue", money(stats.revenue), "", void 0, "revenue", "info"),
          metric("Profit", moneySigned(profit), "", profit >= 0 ? "good" : "bad", "profit", profit >= 0 ? "tech" : "bad"),
          metric("Customers", count(stats.customers), "", void 0, "people", "accent"),
          metric("Turned away", count(stats.lostCustomers), "", lostShare > 0.15 ? "bad" : void 0, "warning", lostShare > 0.15 ? "bad" : "warn")
        ),
        h(
          "div",
...
function metric(label, value, change, tone, glyph, category) {
  return kpi({
    label,
    value,
    sub: change || void 0,
    ...
    valueTone: tone,
    subTone: change.startsWith("\u25B2") ? "good" : change.startsWith("\u25BC") ? "bad" : void 0
  });
}
