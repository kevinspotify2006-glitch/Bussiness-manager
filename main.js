...
function hourlyCapacity(state, business) {
  ...
  return Math.min((power.capacityPerHour + owner) * throughput, floorLimit);
}
...
function contractsWeekly(state) {
  for (const contract of contractsOf(state)) {
    ...
    if (taken >= wanted) {
      ...
      post(state, state.playerCompanyId, "other", `${contract.client} \u2014 shortfall penalty`, -contract.penalty, null);
      contract.missed += 1;
      contract.weeksDone += 1;
      ...
      pushAlert(
        ...
        `${taken} of ${wanted} ${def?.name.toLowerCase() ?? "units"} went out. ${money(contract.penalty)} penalty, ${contract.missed} of ${STRIKES} strikes.`,
        null,
        "contracts"
      );
    }
