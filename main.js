...
var GRADES = [
  ...
  { n: 2, name: "Senior", leadership: 0, skill: 58, pay: 1.38, span: 0, managerial: false, detail: "The one the others ask. As far as the trade alone goes." },
  { n: 3, name: "Team Lead", leadership: 45, skill: 58, pay: 1.6, span: 5, managerial: false, detail: "Still on the floor, and now answerable for a handful of people." },
  { n: 4, name: "Manager", leadership: 60, skill: 62, pay: 2.05, span: 11, managerial: true, detail: "Runs the place. A different job from the one below it." },
  { n: 5, name: "Senior Manager", leadership: 72, skill: 70, pay: 2.7, span: 20, managerial: true, detail: "Runs managers, which is harder than running staff." },
  { n: 6, name: "Executive", leadership: 82, skill: 76, pay: 3.6, span: 34, managerial: true, detail: "Takes a division off your hands entirely." }
];
function grade(n) {
...
function titleOf(employee) {
  ...
  if (def.n === 3) return `Team Lead \u2014 ${roleName}`;
  return def.name;
}
...
function gradeFactor(employee) {
  return 1 + gradeOf(employee).n * 0.035;
}
...
function generateApplicant(state, forcedRole) {
  ...
  if (roleDef.id !== "manager") startGrade = Math.min(startGrade, 2);
  else startGrade = Math.max(4, Math.min(startGrade, 5));
...
function employeesDaily(state) {
  for (const employee of [...state.employees]) {
    ...
    const settlesAt = clamp(
      68 + payEffect * 1.6 + moraleDrift * 10 + fromColleagues * 8 - Math.max(0, employee.workload - 60) * 0.5 - employee.stress * 0.22,
      0,
      100
    );
...
function ownerAttention(state) {
  ...
  const delegated = sum(state.divisions, (division) => {
    ...
    const relief = clamp(performanceOf(head) / 80, 0.35, 1) * clamp(gradeOf(head).span / GRADES[6].span, 0.4, 1);
    return locations * (1 - relief * 0.9);
  });
  const load = direct + delegated;
  if (load <= OWNER_ATTENTION) return 1;
  return clamp(OWNER_ATTENTION / load, 0.55, 1);
}
...
function eligibleHeads(state) {
  return state.employees.filter((person) => person.companyId === state.playerCompanyId && gradeOf(person).n >= 5).sort((a, b) => performanceOf(b) - performanceOf(a));
}
...
function adminCostPerDay(state, business) {
  ...
  return size.adminPerHead * headcount * complexityOf(type) * coordinationFactor(state) * stretched;
}
...
function people(input) {
  ...
  return input.filter(isRecord).map((entry) => {
    ...
    if (typeof entry.grade !== "number") {
      employee.grade = employee.role === "manager" ? 4 : Math.min(2, num(entry.promotions, 0) + (employee.skill >= 58 ? 1 : 0));
    } else {
      employee.grade = Math.max(0, Math.min(6, num(entry.grade, 0)));
    }
    return employee;
  });
}
...
function makeCompanyOffer(state, companyId, amount) {
  ...
  const threshold = valuation.reserve * distress * gameRng.range(0.97, 1.05);
  if (amount < threshold) {
...
function holdingView(ctx) {
  ...
  const render = () => {
    ...
    attentionPanel.appendChild(
      h("p", {
        ...
        style: "margin:8px 0 0",
        text: attention >= 1 ? `Comfortable. One person carries about ${OWNER_ATTENTION} locations before anything starts slipping, and you are inside that.` : `Stretched. One person carries about ${OWNER_ATTENTION} locations; past that everything still happens and every location pays about ${Math.round(
          (attentionFactor(state) - 1) * 100
        ...
        h(
          ...
          button("Start a new division", () => {
            askForName("Name the new division", "New division", (name) => {
              const made = createDivision(state, name, "grouped by you");
  ...
  const signature = () => {
    ...
    return [
      state.day,
      state.divisions.map((d) => `${d.id}:${d.headEmployeeId ?? ""}`).join(","),
